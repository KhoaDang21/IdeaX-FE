...
const Payments: React.FC = () => {
  ...
  const { wallet, transactions, transactionsPage } = useSelector(
    (state: RootState) => state.payment
  );

  // ✅ 2. LẤY signedProjects THAY VÌ projects
  const { signedProjects } = useSelector((state: RootState) => state.project);
  ...
  const fetchWallet = useCallback(async () => {
    ...
    try {
      ...
      message.error(
        extractErrorMessage(err, "Unable to load wallet information")
      );
    } finally {
  ...
  const fetchTransactions = useCallback(
    async (page: number, size: number) => {
      ...
      try {
        ...
        message.error(
          extractErrorMessage(err, "Unable to load transaction history")
        );
      } finally {
  ...
  // ✅ 3. SỬA FETCH PROJECT: Gọi API lấy dự án đã ký
  const fetchProjects = useCallback(async () => {
    try {
      await dispatch(getSignedProjects()).unwrap();
    } catch (err: any) {
      // Có thể log lỗi nhưng không cần hiện message ầm ĩ nếu chỉ là danh sách trống
      console.error("Failed to load signed projects", err);
    }
  }, [dispatch]);
  ...
  // ✅ 4. LIST DỰ ÁN ĐẦU TƯ: Lấy từ signedProjects
  ...
  const handleDepositSubmit = async () => {
    try {
      ...
      message.error(
        extractErrorMessage(err, "Failed to create deposit request")
      );
    } finally {
      setDepositing(false);
    }
  ...
  const handleWithdrawSubmit = async () => {
    try {
      ...
      if (amount < minWithdraw) {
        message.error(
          `Minimum withdraw amount is ${formatCurrency(minWithdraw)}`
        );
        return;
      }
      if (amount > balance) {
      ...
      message.error(
        extractErrorMessage(err, "Failed to create withdraw request")
      );
    } finally {
  ...
  const handleRefundSubmit = async () => {
    ...
    try {
      ...
      message.error(
        extractErrorMessage(err, "Failed to submit refund request")
      );
    } finally {
      setRefunding(false);
    }
      ...
      >
        ...
        <Space size={[12, 12]} wrap style={{ marginTop: 20 }}>
          ...
          <Button
            ...
            // Nếu không có dự án nào đã ký thì disable nút này
  ...
  const renderPendingRequests = () => (
    ...
    >
          ...
          >
            <div style={{ flex: 1 }}>
              ...
              <div style={{ color: "#666", fontSize: 12, marginTop: 4 }}>
                ...
                {dayjs(refund.createdAt).add(3, "day").format("DD/MM/YYYY")}
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
        ...
        >
          ...
          >
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ color: "#666", fontSize: 12 }}>
                Requested: {formatDateTime(withdrawal.createdAt)}
              </div>
            </div>
          </div>
          <Button
      ...
      {!pendingRequests.deposits.length &&
        !pendingRequests.withdrawals.length &&
        !pendingRequests.refunds.length && (
          <Empty
  ...
  return (
    ...
    >
      ...
      {/* ✅ 5. MODAL INVEST ĐÃ ĐƯỢC CẬP NHẬT LOGIC TÌM TRONG SIGNED PROJECTS */}
      <Modal
      ...
      >
        <Form form={investForm} layout="vertical">
          ...
          >
            <Select
              showSearch
              placeholder="Select a signed project to invest"
            ...
            rules={[
              ...
              ({ getFieldValue }) => ({
                validator(_, value) {
                  ...
                  // ✅ QUAN TRỌNG: Tìm project trong signedProjects
                  ...
                  const fundingReceived = Number(
                    (project as any).fundingReceived ?? 0
                  );
                  const remaining = Math.max(
                    fundingAmount - fundingReceived,
                    0
                  );
                  ...
                  if (fundingAmount > 0 && amount > remaining) {
                    return Promise.reject(
                      new Error(
                        `Cannot invest more than remaining funding (${formatCurrency(
                          remaining
                        )})`
                      )
                    );
                  ...
                  const projectMin = Number(
                    (project as any).minimumInvestment ?? 0
                  );
                  if (projectMin > 0 && amount < projectMin) {
                    return Promise.reject(
                      new Error(
                        `Project minimum investment is ${formatCurrency(
                          projectMin
                        )}`
                      )
                    );
          ...
          <Form.Item shouldUpdate={(prev, curr) => prev !== curr}>
            {() => {
              ...
              if (!project)
                return <Text type="secondary">Project info not found</Text>;
              ...
              return (
                ...
                >
                  ...
                  <div style={{ marginTop: 8, fontSize: 13 }}>
                    <div>Target: {formatCurrency(fundingAmount)}</div>
                    <div>Received: {formatCurrency(fundingReceived)}</div>
                    <div style={{ color: remaining === 0 ? "red" : "green" }}>
                      Remaining: {formatCurrency(remaining)}
                    </div>
                    {projectMin > 0 && (
                      <div>Min Investment: {formatCurrency(projectMin)}</div>
                    )}
                  </div>
                </div>
              );
        ...
        {!investableProjects.length && (
          <Text type="secondary">
            No SIGNED projects found. Please sign a contract first.
          </Text>
        )}
      </Modal>
