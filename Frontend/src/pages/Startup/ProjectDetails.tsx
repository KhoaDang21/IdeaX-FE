...
const ProjectDetails: React.FC = () => {
  ...
  if (status === 'loading') {
    return (
      <div style={{ padding: 24, background: '#f9fafb', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <div style={{ color: '#64748b', fontSize: 16 }}>Loading project details...</div>
      </div>
    );
  }
    ...
    return (
      <div style={{ padding: 24, background: '#f9fafb', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <div style={{ textAlign: 'center', maxWidth: 400 }}>
          <p style={{ color: '#ef4444', marginBottom: 16 }}>{error || 'Project not found'}</p>
          <button
            onClick={handleBack}
            style={{
              padding: '8px 16px',
  ...
  const documentItems = [
    { 
      ...
      fileName: getFileName(getFileUrlByType(project.files || [], 'PITCH_DECK'))
    },
    { 
      key: 'pitchVideo', 
      label: 'Pitch Video', 
      ...
      fileName: getFileName(getFileUrlByType(project.files || [], 'PITCH_VIDEO'))
    },
    { 
      key: 'businessPlan', 
      label: 'Business Plan', 
      ...
      fileName: getFileName(getFileUrlByType(project.files || [], 'BUSINESS_PLAN'))
    },
    { 
      ...
      fileName: getFileName(getFileUrlByType(project.files || [], 'FINANCIAL_PROJECTION'))
    },
  ];

  return (
    <div style={{ padding: 24, background: 'linear-gradient(135deg, #f9fafb 0%, #e0e7ff 100%)', minHeight: '100vh' }}>
      <Card
        style={{
          borderRadius: 12,
      ...
      <Row gutter={[24, 24]}>
        <Col xs={24} lg={16}>
          <Card
            title={
              <span style={{ color: '#1e40af', fontSize: 18, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <Row gutter={[16, 16]}>
              <Col span={24}>
                <div style={{ marginBottom: 16 }}>
                  <label style={{ color: '#64748b', fontWeight: 500 }}>Project Name</label>
                  <p style={{ color: '#1e293b', fontSize: 16 }}>{project.projectName || 'N/A'}</p>
                </div>
              </Col>
              <Col xs={24} sm={12}>
                <div style={{ background: '#f9fafb', padding: 12, borderRadius: 8, border: '1px solid #e2e8f0' }}>
                  <label style={{ color: '#64748b', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <Col xs={24} sm={12}>
                <div style={{ background: '#f9fafb', padding: 12, borderRadius: 8, border: '1px solid #e2e8f0' }}>
                  ...
                  <p style={{ color: '#1e293b' }}>{project.fundingStage || 'N/A'}</p>
                </div>
              </Col>
              <Col xs={24} sm={12}>
                <div style={{ background: '#f9fafb', padding: 12, borderRadius: 8, border: '1px solid #e2e8f0' }}>
                  ...
                  <p style={{ color: '#1e293b' }}>{getFundingRangeDisplay(project.fundingRange || '')}</p>
                </div>
              </Col>
              <Col xs={24} sm={12}>
                <div style={{ background: '#f9fafb', padding: 12, borderRadius: 8, border: '1px solid #e2e8f0' }}>
                  ...
                  <p style={{ color: '#1e293b' }}>{project.teamSize || 'N/A'}</p>
                </div>
              </Col>
              <Col xs={24} sm={12}>
                <div style={{ background: '#f9fafb', padding: 12, borderRadius: 8, border: '1px solid #e2e8f0' }}>
                  ...
                  <p style={{ color: '#1e293b' }}>{project.location || 'N/A'}</p>
                </div>
              </Col>
              <Col xs={24} sm={12}>
                <div style={{ background: '#f9fafb', padding: 12, borderRadius: 8, border: '1px solid #e2e8f0' }}>
              ...
              <Col span={24}>
                <div style={{ background: '#f9fafb', padding: 12, borderRadius: 8, border: '1px solid #e2e8f0' }}>
                  ...
                  <p style={{ color: '#1e293b', whiteSpace: 'pre-wrap' }}>{project.description || 'N/A'}</p>
                </div>
              </Col>
            </Row>
          </Card>
                ...
                <Col xs={24} key={item.key}>
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ fontSize: 24, color: '#1e40af' }}>{item.icon}</div>
                      <div style={{ flex: 1 }}>
                        <p style={{ margin: 0, color: '#1e293b', fontWeight: 500 }}>{item.label}</p>
                        <p style={{ margin: 4, color: '#64748b', fontSize: 12 }}>{item.desc}</p>
